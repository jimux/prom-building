...
/*
 * Use the function version of tolower (from strcasecmp.c) to avoid
 * double-evaluation of side-effecting arguments like *s2++ in nvmatch().
 * The macro version evaluates its argument twice (once in isupper(), once
 * in the selected ternary branch), breaking post-increment usage.
 */
#undef tolower
