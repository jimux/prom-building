...
/*
 * ABI/ISA override for bcopy/bzero block copy:
 *
 * The sys/asm.h macros set SZREG=8, REG_L=ld, REG_S=sd when _MIPS_ISA is
 * MIPS3 or MIPS4.  However, with O32 ABI (-mabi=32), the GNU assembler
 * rejects 64-bit ld/sd instructions using 32-bit register names (e.g.
 * "ld t0,0(a0)" is invalid in O32 mode).  The compiler silently generates
 * lw/sw (4-byte) instructions instead, but SZREG and BLKSIZE remain 8 and
 * 64, causing the 8-register loop template to actually emit 16 lw loads
 * that reuse registers, corrupting every other 4-byte word of output.
 *
 * The fix: when _MIPS_SZPTR==32 (O32 ABI), override SZREG to 4 and use
 * lw/sw unconditionally.  The block size becomes 32 bytes per iteration
 * instead of 64, which is correct and safe.
 *
 * Reference: sys/asm.h lines 786-789 note exactly this scenario:
 *   "For Everest and Crimson, the assembly code is compiled -mips3 -32bit
 *    ...we would then have double-word ops in a 32 bit kernel, which is
 *    not desirable."
 */
#if (_MIPS_SZPTR == 32)
#undef  SZREG
