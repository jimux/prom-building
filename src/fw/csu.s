 ...
 */
     ...
     * The linker places initialized data in the PROM image at _data_load
     * but it runs from RAM at _fdata.  Without this copy all initialized
     * globals (prom_menu, cmd_table, string pointers, …) are zero.
     */
    la      t0, _fdata            /* dst: RAM VMA  (0x80100000)         */
    la      t1, _data_load        /* src: ROM LMA  (0xBFC10C5C)         */
    la      t2, _edata            /* end: RAM VMA  (0x8010071C)         */
    beq     t0, t2, _data_done    /* skip if .data is empty             */
    nop
_data_copy:
    lw      t3, 0(t1)             /* load word from ROM                 */
    sw      t3, 0(t0)             /* store word to RAM                  */
    addiu   t0, t0, 4
    addiu   t1, t1, 4
    bne     t0, t2, _data_copy
    nop
_data_done:

    /*
     * Zero from firstBss to PROM_STACK-64.
     * PROM_STACK is defined in kseg1 (0xA1400000) but firstBss is linked
     * in kseg0 (0x80xxxxxx). Converting PROM_STACK to kseg0 before
     * subtraction avoids wrap-around.
     */
    la      a0, firstBss          /* kseg0 address of BSS start         */
    li      t0, (((PROM_STACK) & 0x1FFFFFFF) | 0x80000000) - 64
    jal     bzero                 /* bzero(firstBss, size)               */
    subu    a1, t0, a0            /* size = kseg0_end - kseg0_start      */
#endif
